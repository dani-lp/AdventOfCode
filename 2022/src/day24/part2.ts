...
// algorithm
const getPathCost = (initialPosition: Point, finalPosition: Point, blizzards: Blizzard[]): [number, Blizzard[]] => {
  let round = 0;
  let end = false;
  ...
  return [round, currentBlizzards];
};

const [round1, blizzardsAfter1] = getPathCost(initialPosition, finalPosition, blizzards);
const [round2, blizzardsAfter2] = getPathCost(finalPosition, initialPosition, blizzardsAfter1);
const [round3] = getPathCost(initialPosition, finalPosition, blizzardsAfter2);

// output
const result = round1 + round2 + round3;
console.assert(result === 54, `Expected 18, got ${result}`);
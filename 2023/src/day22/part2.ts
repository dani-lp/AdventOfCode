...
const canDisintegrateCount = bricks
  .map(({ id }) => {
    let otherBricks = bricks.filter((brick) => brick.id !== id);
    const bricksFallen = new Map<string, boolean>(
      otherBricks.map(({ id }) => [id, false]),
    );

    while (true) {
      let nextBricks: Brick[] = [];
      const space = generateSpace(otherBricks);

      let falls = 0;
      otherBricks.forEach((brick) => {
        if (brick.shape.some(({ z }) => z === 1)) {
          // already at the bottom
          nextBricks.push({
            id: brick.id,
        ...
        if (canFall) {
          falls++;
          bricksFallen.set(brick.id, true);
          nextBricks.push({
            id: brick.id,
            shape: brick.shape.map(({ x, y, z }) => ({ x, y, z: z - 1 })),
          });
      ...
      otherBricks = nextBricks;

      if (falls === 0) {
        break;
      }
    }
    return Array.from(bricksFallen.values()).filter(Boolean).length;
  })
  .reduce((a, b) => a + b, 0);

assertResult(7, canDisintegrateCount);

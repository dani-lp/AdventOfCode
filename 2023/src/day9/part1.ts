...
const result = input
  ...
  .map((baseSequenceList) => {
    ...
    for (let i = sequenceList.length - 1; i > 0; i--) {
      ...
      const lastBaseSequenceValue = baseSubsequence[baseSubsequence.length - 1];
      const lastTargetSequenceValue =
        targetSubsequence[targetSubsequence.length - 1];
      if (
        lastBaseSequenceValue === undefined ||
        lastTargetSequenceValue === undefined
      ) {
        console.error("No target value found");
        process.exit(1);
      }

      targetSubsequence.push(lastTargetSequenceValue + lastBaseSequenceValue);
    }
  ...
  .map((sequenceList) => {
    ...
    return firstList[firstList.length - 1]!;
  })
  .reduce((a, b) => a + b, 0);
assertResult(114, result);
